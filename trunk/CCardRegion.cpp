...
void CCardRegion::DrawCardStack(SDL_Surface *s)
{
	...
	for(std::vector<CCard>::iterator vi = InternalStack.begin();
			vi != InternalStack.end(); ++vi)
	{
		if(vi->FaceUp())
...
{
	for(std::vector<CCard>::iterator vi = InternalStack.begin(); vi != InternalStack.end(); ++vi)
	{
		if(AnimateCard(*vi) == -1)
			return;
	}
...
bool CCardRegion::CanDrop(CCardStack *stack)
{
	...
	std::vector<CCard>::iterator vi = stack->begin();
	...
	std::vector<CCard>::iteratormyvi = InternalStack.end()-1;
...
int CCardRegion::GetStackWidth()
{
	...
	return (InternalStack[InternalStack.Size()-1].x + CARD_WIDTH) - InternalStack[0].x;
...
int CCardRegion::GetStackHeight()
{
	...
	return (InternalStack[InternalStack.Size()-1].y + CARD_HEIGHT) - InternalStack[0].y;
